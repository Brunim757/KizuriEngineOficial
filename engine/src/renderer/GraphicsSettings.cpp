...
void GraphicsSettings::TuneToHardware() {
    ...
    SSGIEnabled = false; SSGIIntensity = 0.4f;
    CloudsEnabled = false;
    LensFlareEnabled = false; LensFlareIntensity = 0.6f;
    FXAAEnabled = false;
...
void GraphicsSettings::Clamp() {
    ...
    SSGIIntensity = std::clamp(SSGIIntensity, 0.0f, 2.0f);
    LensFlareIntensity = std::clamp(LensFlareIntensity, 0.0f, 3.0f);
    Saturation = std::clamp(Saturation, 0.0f, 2.0f);
    Contrast = std::clamp(Contrast, 0.0f, 2.0f);
    BloomAnamorphic = std::clamp(BloomAnamorphic, 0.0f, 1.0f);
    FogHeightFalloff = std::clamp(FogHeightFalloff, 0.0f, 200.0f);
...
void GraphicsSettings::ApplyPreset(QualityPreset preset) {
    switch (preset) {
        ...
        case QualityPreset::Medium:
            ...
            SSGIEnabled = false; SSGIIntensity = 0.3f;
            CloudsEnabled = false;
        ...
        case QualityPreset::Low:
            ...
            SSGIEnabled = false; SSGIIntensity = 0.0f;
            CloudsEnabled = false;
            LensFlareEnabled = false; LensFlareIntensity = 0.0f;
            FXAAEnabled = false; Saturation = 1.0f; Contrast = 1.0f; BloomAnamorphic = 0.0f;
...
bool SaveGraphicsSettings(const std::string& path, const GraphicsSettings& settings) {
    ...
    j["ssgi_enabled"] = settings.SSGIEnabled;
    j["ssgi_intensity"] = settings.SSGIIntensity;
    j["clouds_enabled"] = settings.CloudsEnabled;
    j["lens_flare_enabled"] = settings.LensFlareEnabled;
    j["lens_flare_intensity"] = settings.LensFlareIntensity;
    j["fxaa_enabled"] = settings.FXAAEnabled;
    j["saturation"] = settings.Saturation;
    j["contrast"] = settings.Contrast;
...
bool LoadGraphicsSettings(const std::string& path, GraphicsSettings& out) {
    ...
    try {
        ...
        out.SSGIEnabled = j.value("ssgi_enabled", out.SSGIEnabled);
        out.SSGIIntensity = j.value("ssgi_intensity", out.SSGIIntensity);
        out.CloudsEnabled = j.value("clouds_enabled", out.CloudsEnabled);
        out.LensFlareEnabled = j.value("lens_flare_enabled", out.LensFlareEnabled);
        out.LensFlareIntensity = j.value("lens_flare_intensity", out.LensFlareIntensity);
        out.FXAAEnabled = j.value("fxaa_enabled", out.FXAAEnabled);
        out.Saturation = j.value("saturation", out.Saturation);
        out.Contrast = j.value("contrast", out.Contrast);
