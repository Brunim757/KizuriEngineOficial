...
void GraphicsSettings::TuneToHardware() {
    // Extrai o MÁXIMO de cada versão: 3.3 = MSAA 4x sólido; 4.0+ sobe pra 8x
    // (o bug de depth-resolve do MSAA era do Wine, não de GPU real). O AO
    // clampado em 0.35 garante que o SSAO nunca escureça a cena mesmo se a
    // estimativa quebrar num driver estranho.
    int glsl = GetGLSLVersion();
    // SSR (reflexos por raio) precisa de GL 4.0+ (loop variável): em 3.3 fica
    // desligado; de 4.0 pra cima liga, com mais passos quanto maior a versão.
    SSREnabled = (glsl >= 400);
    if (glsl >= 450) {
        ...
        MSAA = 8; ShadowMapSize = 2048; ShadowPCFRadius = 3; SSAOSamples = 48;
...
void GraphicsSettings::Clamp() {
    ...
    SSRMaxSteps = std::clamp(SSRMaxSteps, 8, 128);
...
void GraphicsSettings::ApplyPreset(QualityPreset preset) {
    switch (preset) {
        case QualityPreset::Ultra:
            ...
            SSREnabled = true;    SSRIntensity = 0.7f;   SSRMaxSteps = 64;
        ...
        case QualityPreset::High:
            ...
            SSREnabled = true;    SSRIntensity = 0.6f;   SSRMaxSteps = 48;
            Exposure = 1.0f;      VSync = true;
            FogEnabled = false;   FogDensity = 0.012f;
            FogColor[0] = 0.55f;  FogColor[1] = 0.6f;  FogColor[2] = 0.66f;
        ...
        case QualityPreset::Medium:
            ...
            SSREnabled = true;    SSRIntensity = 0.45f;  SSRMaxSteps = 32;
            Exposure = 1.0f;      VSync = true;
            FogEnabled = false;   FogDensity = 0.012f;
            FogColor[0] = 0.55f;  FogColor[1] = 0.6f;  FogColor[2] = 0.66f;
        ...
        case QualityPreset::Low:
            ...
            SSREnabled = false;   SSRIntensity = 0.0f;   SSRMaxSteps = 16;
